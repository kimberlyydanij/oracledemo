...
/*-----------------------------------------------------------
1. equi join 
   가장 많이 사용되는 조인 방법으로 조인 대상이 되는 두 테이블에서 공통적으로 존재하는
...
-- 직원들의 first_name, hire_date, department_name, city를
-- 출력하는 SELECT를 작성하시오.
...
/*----------------------------------------
2. certeian product(카티션 곱) 조인 : 
    테이블 행의 갯수만큼 출력해주는 조인이다.
    --------------------------------------*/
-- 1) oracle 용 certeian product
...
3. NATURAL JOIN
    ...
    ORACLE에서는 가능하나 SQL Server에서는 지원하지 않는 기능이다.
---------------------------------------------------------*/
...
6. SELF JOIN
    ...
    하나의 테이블에 같은 데이터가 두개의 컬럼에 다른 목적으로 저장도어 있는 경우
    employees, imployee_id, manager_id
------------------------------------------------*/
