...
-- 날짜 -> 문자
SELECT to_char(sysdate, 'yyyy-mm-dd hh:mi:ss day')  /*12 시간,  day:월요일*/
FROM dual;

SELECT to_char(sysdate, 'yyyy-mm-dd hh:mi:ss dy')  /*12 시간, dy: 월*/
FROM dual;

SELECT to_char(sysdate, 'yyyy-mm-dd hh24:mi:ss day')  /*24 시간, dy:월*/
FROM dual;
/*
java
//HH : 24시간, hh:12시간
...
--hire_date(입사일)을 이용해서 '홍길동님은 2003년 1월 3일에 입사했습니다.'로 출력한는 query을 작성하시요.
...
-- salary의 값이 10000이만이면 '하', 10000이상 20000 미만이면 '중', 20000이상이면 '상'
...
-- 부서별 최소급여, 최대급여를 최소값와 최대값이 같이 않은 경우에만 부서별 오름차순으로 출력하시오.
