...
--6) employees 테이블에서 직종이 'FI_ACCOUNT' 또는 'AC_ACCOUNT' 인 사원들 중 최대급여를  구하는    SELECT문장을 기술하시오.   
   ...
   FROM employees
   GROUP BY job_id
   HAVING (job_id = 'FI_ACCONT' OR job_id = 'AC_ACCOUNT');
   
  SELECT max(salary)
   FROM employees
   WHERE (job_id = 'FI_ACCONT' OR job_id = 'AC_ACCOUNT');
   
   SELECT max(salary)
   FROM employees
   WHERE job_id IN('FI_ACCONT' , 'AC_ACCOUNT');
   
   --WHERE job_id = 'FI_ACCONT' || job_id =  'AC_ACCOUNT'
